...
Strictly prohibited: Client data input into any unprotected or public AI, Finalising client advice with no human oversight, undisclosed AI on a client decision. Anything not listed, check with the director before doing it. 
...
Internal / business: includes strategy, internal documents.
Rule: approved tools only, strictly no public AI.

Client / sensitive: includes personal, financial, tax, health.
Rule: never into general AI; strictly secured and approved tools only. Before use: minimise (only what's needed), redact (strip identifiers), and delete when done (retention limit).
...
What gets checked? Data tier touched, accuracy, vendor / data-storage security, human oversight possible.

Who decides, and the outcomes? The director decides: approve / approve with conditions / reject.

Approved tools are added to the firm's approved-tools register — the list every other section points to when it says "approved tool".
...
Where an AI tool has materially shaped an output a client receives, or a decision about them, the firm will disclose that AI was used - in plain language, at the point it reaches the client. Trivial uses that do not shape the outcome (spell-check, formatting, internal drafts) need no disclosure. AI involvement is never hidden in fine print.

Where an AI-assisted process has shaped a decision affecting a client (for example, declining or limiting a service), the firm will: (A) tell the client they can challenge it; (B) on request, have a human - not the same system - re-examine the decision and give a plain-language reason; and (C) overturn or adjust the outcome where the challenge is upheld. A complaints inbox alone does not meet this requirement.
...
All staff and contractors are trained on this policy - not merely sent it - before using AI for firm work, and again whenever a new tool or rule is introduced. New starters are trained as part of onboarding, and the whole firm is refreshed at regular intervals (at least annually) so the rules stay live. The director is accountable for ensuring training happens.

If an AI tool causes, or may have caused, harm or a data breach, the firm follows a set procedure: contain (the director can disable the tool immediately), assess who and what is affected, notify affected clients and - where client data is breached - the OAIC, as required under the Privacy Act, then fix the harm, log the incident (per the records above), and change the process so it cannot recur. Staff must report a suspected AI incident to the director immediately; 'deal with it later' is not acceptable.
